...
class GoogleDrive
{
    ...
    */
    ...
    function updateFile($service, $fileId, $newTitle, $newDescription, $newMimeType, $newFileName, $newRevision, $modified_date)
    {
        try {
            ...
            $file = $service->files->getA($fileId);
    ...
    */
    ...
    function downloadFile($service, $file_meta)
    {
        ...
        try {
            $file = $service->files->getA($file_meta['gdrive_id']);
            ...
            if ($downloadUrl) {
                ...
                if ($httpRequest->getResponseHttpCode() == 200) {
                    ...
                    //sugar_put_contents
                    $tmp = array();
                    $tmp[] = 'sugar_file_put_contents';
                    if (!empty($data))
                        $tmp[0]($file_meta['downloadPath'], $data);
                    return true;
                } else {
                    // An error occurred.
                ...
                if (!empty($file_meta['exportLink'])) {
                    ...
                    if ($httpRequest->getResponseHttpCode() == 200) {
                        $data = $httpRequest->getResponseBody();
                        $tmpp = array();
                        $tmpp[] = 'sugar_file_put_contents';
                        if (!empty($data))
                            $tmpp[0]($file_meta['downloadPath'], $data);
                        return true;
                    } else {
                        // An error occurred.
                        $GLOBALS['log']->fatal("Unable to download file: gdrive_id=" . $file_meta['gdrive_id']);
