...
//        $GLOBALS['log']->fatal('filterList:    384 /var/www/html/luke_prod/clients/base/api/FilterApi.php');
...
// fill in related rows data by field
        ...
        foreach ($bean->field_defs as $field => $fieldDef) {
            if (in_array($fieldDef['type'], $bean::$relateFieldTypes) && (!empty($fieldDef['link']) || !empty($fieldDef['module']))) {
...
// we have direct data - populate it
                    $rbean->populateFromRow(
                            array($fieldDef['rname'] => $data[$field]), true
                    );
                } else {
...
// next API version
    ...
    SugarQuery $q, SugarQuery_Builder_Where $where, $filter, $joinType = 'LEFT'
    ) {
        $field = 'following';
        $q->select($field);
    ...
    SugarQuery $q, SugarQuery_Builder_Where $where, $link, $joinType = 'LEFT'
    ) {
...
// databases other than MySQL
        $join = $q->joinTable(
                ...
                ' AND t.deleted = 0 ' .
                ' GROUP BY t.item_id)', array('alias' => 'tracker')
        );
