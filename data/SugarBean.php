...
class SugarBean {
    ...
    public function __construct() {
        ...
        if (!$this->disable_row_level_security && !empty($current_user->id) &&
                ...
                ($this->bean_implements('ACL') && (ACLAction::getUserAccessLevel($current_user->id, $this->module_dir, 'access') == ACL_ALLOW_ENABLED && (ACLAction::getUserAccessLevel($current_user->id, $this->module_dir, 'admin') == ACL_ALLOW_ADMIN || ACLAction::getUserAccessLevel($current_user->id, $this->module_dir, 'admin') == ACL_ALLOW_ADMIN_DEV))))) {
    ...
    function getAuditEnabledFieldDefinitions($includeRelateIdFields = false) {
        if (!isset($this->audit_enabled_fields) || !isset($this->auditEnabledRelateFields)) {
            ...
            foreach ($this->field_defs as $field => $properties) {
                if (($field === 'team_id' || !empty($properties['Audited']) || !empty($properties['audited'])) && SugarACL::checkField($this->module_dir, $field, 'access', array('bean' => $this))) {
            ...
            foreach ($indices as $key => $index) {
                if (isset($index['fields'])) {
                    ...
                    foreach ($index['fields'] as $field) {
                        if (isset($fieldDefs[$field]) && isset($fieldDefs[$field]['source']) && $fieldDefs[$field]['source'] == 'non-db'
                        ) {
    ...
    function get_importable_fields() {
        ...
        if (!empty($fieldDefs)) {
            foreach ($fieldDefs as $key => $value_array) {
                if ((isset($value_array['importable']) && (is_string($value_array['importable']) && $value_array['importable'] == 'false' || is_bool($value_array['importable']) && $value_array['importable'] == false)) || (isset($value_array['type']) && $value_array['type'] == 'link') || (isset($value_array['type']) && $value_array['type'] == 'image') || (isset($value_array['auto_increment']) && ($value_array['type'] == true || $value_array['type'] == 'true'))) {
    ...
    protected function addParentRecordsToResave($lname) {
        ...
        if (!empty($this->field_defs['parent_type']) && $this->field_defs['parent_type']['type'] == 'parent_type' && !empty($this->field_defs['parent_id']) && $this->field_defs['parent_id']['type'] == 'id' && !empty($this->fetched_row['parent_type']) && !empty($this->fetched_row['parent_id']) && (!isset($this->parent_id) || $this->parent_id != $this->fetched_row['parent_id'])) {
        ...
        if (!empty($this->field_defs['parent_type']) && $this->field_defs['parent_type']['type'] == 'parent_type' && !empty($this->field_defs['parent_id']) && $this->field_defs['parent_id']['type'] == 'id' && isset($this->parent_id) && $this->parent_id != $this->fetched_row['parent_id'] && isset($this->parent_type) && $this->$lname->getRelatedModuleName() == $this->parent_type
        ) {
            SugarRelationship::addToResaveList(
    ...
    function send_assignment_notifications($notify_user, $admin) {
        if (($this->object_name == 'Meeting' || $this->object_name == 'Call') || (isset($notify_user->receive_notifications) && $notify_user->receive_notifications)) {
        ...
        if (!empty($new_rel_id)) {
            if ($this->load_relationship($new_rel_link)) {
                return $this->$new_rel_link->add(
                                $new_rel_id, isset($this->additional_rel_values) ? $this->additional_rel_values : array()
                );
            } else {
    ...
    public function retrieve($id = -1, $encode = true, $deleted = true) {
        ...
        $query->from(
                $this, ['add_deleted' => $deleted, 'team_security' => !$this->disable_row_level_security, 'action' => 'view']
        );
        $query->select('*');
        ...
        if (!empty($name_format_fields)) {
            ...
            foreach ($this->related_user_fields as $id_field => $params) {
                ...
                if (isset($this->field_defs[$id_field]) && isset($this->field_defs[$name_field]) && ($this->field_defs[$name_field]['type'] == 'relate')
                ) {
...
//            $GLOBALS['log']->fatal('$parameters: ', print_r($parameters, 1));
        ...
        if (isset($options['TTTTTT'])) {
//            $GLOBALS['log']->fatal('$rows: ', print_r($rows, 1));
//
//            $GLOBALS['log']->fatal('$options : ', print_r($options, 1));
        }
        $cnt = 0;
        $rawRows = array();
        ...
        foreach ($rows as $row) {
            ...
            // $GLOBALS['log']->fatal('Bean: ' . $cnt . '  :: ' . $bean->id);
            ...
            if (isset($options['TTTTTT'])) {
                $GLOBALS['log']->fatal('$cnt call_custom_logic : ' . $cnt . ' :: ' . $bean->id);
            }
            $beans[$bean->id] = $bean;
        ...
        if (isset($options['TTTTTT'])) {
            $GLOBALS['log']->fatal('$rawRows: ', count($rawRows));
        }
    ...
    protected function getQueryFields(SugarQuery $query, array $fields = array(), array $options = array()) {
        ...
        foreach ($fields as $field) {
            ...
            if (isset($def['source']) && $def['source'] == 'non-db' && (empty($def['rname']) || empty($def['link'])) && empty($def['db_concat_fields'])) {
    ...
    protected function logDistinctMismatch(array $sqlRows, array $beans) {
        // generic warning
        $msg = sprintf(
                "Non-distinct result set detected: sqlRows = %s vs beanSet = %s", count($sqlRows), count($beans)
        );
    ...
    $order_by, $submodule = null, $suppress_table_name = false, $field_map = array()
    ) {
        if (empty($order_by))
            return $order_by;
    ...
    $parentbean, $order_by = "", $sort_order = '', $where = "", $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0, $subpanel_def
    ) {
    ...
    function process_union_list_query($parent_bean, $query, $row_offset, $limit = -1, $max_per_page = -1, $where = '', $subpanel_def, $query_row_count = '', $secondary_queries = array()) {
        ...
        if ($performSecondQuery) {
            ...
            while (($index < $row_offset + $max_per_page || $max_per_page == -99) && ($row = $db->fetchByAssoc($result))) {
                $data[$row['id']] = $row;
                $index++;
            }
        ...
        foreach ($this->field_defs as $field) {
            ...
            if (empty($this->$name)) {
                if (in_array($field['type'], static::$relateFieldTypes) && !empty($field['module']) && !empty($field['id_name'])) {
                    ...
                    if (empty($this->$id_name) && isset($this->field_defs[$id_name]['type']) && ($this->field_defs[$id_name]['type'] == 'relate' ||
                            ($this->field_defs[$id_name]['type'] == 'id' && isset($this->field_defs[$id_name]['source']) && $this->field_defs[$id_name]['source'] == 'non-db'))) {
                    ...
                    if (!empty($this->$id_name) && ($this->object_name != $related_module || ($this->object_name == $related_module && $this->$id_name != $this->id))) {
                        ...
                        if ($is_full_name_relate) {
                            ...
                            foreach ($name_format_fields as $name_field) {
                                $related_name_fields[$name_field] = $fields[$name_field] = $this->getRelateAlias($name, $name_field);
                            }
                        } else {
                            $fields[$rname] = $name;
                        }
        ...
        if (isset($_SESSION['show_deleted'])) {
            ...
            if (isset($this->field_defs['modified_user_id'])) {
                ...
                $this->db->updateParams(
                        $this->table_name, $this->field_defs, array('deleted' => 1,
                    'date_modified' => $date_modified,
                    'modified_user_id' => $this->modified_user_id), array('id' => $id)
                );
                if ($this->isFavoritesEnabled()) {
                ...
                $this->db->updateParams(
                        $this->table_name, $this->field_defs, array('deleted' => 1,
                    'date_modified' => $date_modified), array('id' => $id)
                );
                if ($this->isFavoritesEnabled()) {
    ...
    public function getRelatedFields($module, $id, $fields, $return_array = false) {
        ...
        foreach ($fields as $field => $alias) {
            if (!empty($GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields'])) {
                $selectFields[] = $this->db->concat($table, $GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields'])
                        . ' AS ' . $alias;
    ...
    $table, array $relate_values, $check_duplicates = true, $do_update = false, array $data_values = array()
    ) {
        global $dictionary;
    ...
    function loadLayoutDefs() {
        ...
        if (empty($this->layout_def)) {
            foreach (SugarAutoLoader::existing("modules/{$this->module_dir}/layout_defs.php", "custom/modules/{$this->module_dir}/Ext/Layoutdefs/layoutdefs.ext.php") as $file) {
                require $file;
            }
    ...
    function isOwner($user_id) {
        ...
        if (isset($this->assigned_user_id)) {
            ...
            if (isset($this->fetched_row['assigned_user_id']) && $this->fetched_row['assigned_user_id'] == $user_id) {
                return true;
            }
            return false;
        } else {
    ...
    public function convertField($fieldvalue, $fieldDef) {
        if (!empty($fieldvalue)) {
            if (!(isset($fieldDef['source']) &&
                    !in_array($fieldDef['source'], array('db', 'custom_fields', 'relate')) && !isset($fieldDef['dbType'])) || (isset($fieldDef['type']) && $fieldDef['type'] == 'relate') || isset($fieldDef['rname_link'])
            ) {
    ...
    public function create_export_query($order_by, $where) {
        ...
        foreach ($fields as $field => $data) {
            ...
            if ($field == 'assigned_user_name' || $field == 'email1' || $field == 'modified_by_name' || $field == 'created_by_name') {
                continue;
            }
        ...
        $returnArray = $this->create_new_list_query(
                $order_by, $where, $filtered_fields, $new_list_params, 0, '', true, $this, true, true, true
        );
    ...
    public function hasEmails() {
        return isset($this->field_defs['email_addresses']['type']) && $this->field_defs['email_addresses']['type'] == 'link';
    }
    ...
    public function getRelateFieldQuery($field_def, $joinTableAlias, $selectedFields = array()) {
        ...
        if (isset($this->field_defs[$rname])) {
            ...
            $sort_fields = $this->getRelateSortColumns(
                    $rname_field_def, $joinTableAlias, $joinCustomTableAlias, $has_custom_fields
            );
        }
    ...
    public function getLockedFieldRelBeans() {
        ...
        if ($this->id && ($relField = $this->getLockedFieldRelField()) && PMSEEngineUtils::doesModuleHaveLockedFields($this->getModuleName())
        ) {
