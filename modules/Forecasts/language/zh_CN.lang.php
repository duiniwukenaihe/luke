...
$mod_strings = array (
    ...
    'LBL_FORECASTS_CONFIG_HELP_RANGES' => '配置您想如何对 {{forecastByModule}} 分类。<br><br>请注意，首次提交后无法更改范围设置。对于升级后的实例，范围设置将使用现有预测数据被锁定。<br><br>您可以选择根据可能性范围创建两个或多个类别，或者不根据可能性创建类别。<br><br>在自定义类别的左侧有复选框；使用这些复选框来确定哪些范围将纳入提交且报告至经理的预测金额内。<br><br>用户可以从自己的工作表手动更改 {{forecastByModule}} 的包含/排除状态和类别。',
