<?php
/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */

$mod_strings = array (
  // Dashboard Names
  'LBL_TARGETS_LIST_DASHBOARD' => 'แดชบอร์ดรายการเป้าหมาย',
  'LBL_TARGETS_RECORD_DASHBOARD' => 'แดชบอร์ดบันทึกเป้าหมาย',

  'LBL_MODULE_NAME' => 'เป้าหมาย',
  'LBL_MODULE_NAME_SINGULAR' => 'เป้าหมาย',
  'LBL_MODULE_ID'   => 'เป้าหมาย',
  'LBL_INVITEE' => 'ผู้ใต้บังคับบัญชา',
  'LBL_MODULE_TITLE' => 'เป้าหมาย: หน้าแรก',
  'LBL_SEARCH_FORM_TITLE' => 'การค้นหาเป้าหมาย',
  'LBL_LIST_FORM_TITLE' => 'รายการเป้าหมาย',
  'LBL_NEW_FORM_TITLE' => 'เป้าหมายใหม่',
  'LBL_PROSPECT' => 'เป้าหมาย:',
  'LBL_BUSINESSCARD' => 'นามบัตร',
  'LBL_LIST_NAME' => 'ชื่อ',
  'LBL_LIST_LAST_NAME' => 'นามสกุล',
  'LBL_LIST_PROSPECT_NAME' => 'ชื่อเป้าหมาย',
  'LBL_LIST_TITLE' => 'ชื่อเรื่อง',
  'LBL_LIST_EMAIL_ADDRESS' => 'อีเมล',
  'LBL_LIST_OTHER_EMAIL_ADDRESS' => 'อีเมลอื่น',
  'LBL_LIST_PHONE' => 'โทรศัพท์',
  'LBL_LIST_PROSPECT_ROLE' => 'บทบาท',
  'LBL_LIST_FIRST_NAME' => 'ชื่อ',
  'LBL_ASSIGNED_TO_NAME' => 'ระบุให้',
  'LBL_ASSIGNED_TO_ID'=>'ระบุให้:',
//DON'T CONVERT THESE THEY ARE MAPPINGS
  'db_last_name' => 'LBL_LIST_LAST_NAME',
  'db_first_name' => 'LBL_LIST_FIRST_NAME',
  'db_title' => 'LBL_LIST_TITLE',
  'db_email1' => 'LBL_LIST_EMAIL_ADDRESS',
  'db_email2' => 'LBL_LIST_OTHER_EMAIL_ADDRESS',
//END DON'T CONVERT
  'LBL_EXISTING_PROSPECT' => 'ใช้ที่อยู่ติดต่อที่มีอยู่',
  'LBL_CREATED_PROSPECT' => 'สร้างที่อยู่ติดต่อใหม่แล้ว',
  'LBL_EXISTING_ACCOUNT' => 'ใช้บัญชีที่มีอยู่',
  'LBL_CREATED_ACCOUNT' => 'สร้างบัญชีใหม่แล้ว',
  'LBL_CREATED_CALL' => 'สร้างการโทรใหม่แล้ว',
  'LBL_CREATED_MEETING' => 'สร้างการประชุมใหม่แล้ว',
  'LBL_ADDMORE_BUSINESSCARD' => 'เพิ่มนามบัตรอื่น',
  'LBL_ADD_BUSINESSCARD' => 'ป้อนนามบัตร',
  'LBL_NAME' => 'ชื่อ:',
  'LBL_FULL_NAME' => 'ชื่อ',
  'LBL_PROSPECT_NAME' => 'ชื่อเป้าหมาย:',
  'LBL_PROSPECT_INFORMATION' => 'ภาพรวม',
  'LBL_MORE_INFORMATION' => 'ข้อมูลเพิ่มเติม',
  'LBL_FIRST_NAME' => 'ชื่อ:',
  'LBL_OFFICE_PHONE' => 'โทรศัพท์ที่ทำงาน:',
  'LBL_ANY_PHONE' => 'โทรศัพท์ใดก็ได้:',
  'LBL_PHONE' => 'โทรศัพท์:',
  'LBL_LAST_NAME' => 'นามสกุล:',
  'LBL_MOBILE_PHONE' => 'อุปกรณ์เคลื่อนที่:',
  'LBL_HOME_PHONE' => 'บ้าน:',
  'LBL_OTHER_PHONE' => 'โทรศัพท์อื่น:',
  'LBL_FAX_PHONE' => 'โทรสาร:',
  'LBL_STREET' => 'ถนน',
  'LBL_PRIMARY_ADDRESS_STREET' => 'ถนนของที่อยู่หลัก:',
  'LBL_PRIMARY_ADDRESS_CITY' => 'เมืองของที่อยู่หลัก:',
  'LBL_PRIMARY_ADDRESS_COUNTRY' => 'ประเทศของที่อยู่หลัก:',
  'LBL_PRIMARY_ADDRESS_STATE' => 'รัฐของที่อยู่หลัก:',
  'LBL_PRIMARY_ADDRESS_POSTALCODE' => 'รหัสไปรษณีย์ของที่อยู่หลัก:',
  'LBL_ALT_ADDRESS_STREET' => 'ถนนของที่อยู่รอง:',
  'LBL_ALT_ADDRESS_CITY' => 'เมืองของที่อยู่รอง:',
  'LBL_ALT_ADDRESS_COUNTRY' => 'ประเทศของที่อยู่รอง:',
  'LBL_ALT_ADDRESS_STATE' => 'รัฐของที่อยู่รอง:',
  'LBL_ALT_ADDRESS_POSTALCODE' => 'รหัสไปรษณีย์ของที่อยู่รอง:',
  'LBL_TITLE' => 'ชื่อ:',
  'LBL_DEPARTMENT' => 'แผนก:',
  'LBL_BIRTHDATE' => 'วันเกิด:',
  'LBL_EMAIL_ADDRESS' => 'ที่อยู่อีเมล:',
  'LBL_OTHER_EMAIL_ADDRESS' => 'อีเมลอื่น:',
  'LBL_ANY_EMAIL' => 'อีเมล:',
  'LBL_ASSISTANT' => 'ผู้ช่วย:',
  'LBL_ASSISTANT_PHONE' => 'โทรศัพท์ของผู้ช่วย:',
  'LBL_DO_NOT_CALL' => 'ไม่ติดต่อ:',
  'LBL_EMAIL_OPT_OUT' => 'ปฏิเสธการรับอีเมล:',
  'LBL_PRIMARY_ADDRESS' => 'ที่อยู่หลัก:',
  'LBL_ALTERNATE_ADDRESS' => 'ที่อยู่อื่น:',
  'LBL_ANY_ADDRESS' => 'ที่อยู่ใดก็ได้:',
  'LBL_CITY' => 'เมือง:',
  'LBL_STATE' => 'รัฐ:',
  'LBL_POSTAL_CODE' => 'รหัสไปรษณีย์:',
  'LBL_COUNTRY' => 'ประเทศ:',
  'LBL_DESCRIPTION_INFORMATION' => 'ข้อมูลคำอธิบาย',
  'LBL_ADDRESS_INFORMATION' => 'ข้อมูลที่อยู่',
  'LBL_DESCRIPTION' => 'คำอธิบาย:',
  'LBL_PROSPECT_ROLE' => 'บทบาท:',
  'LBL_OPP_NAME' => 'ชื่อโอกาสทางการขาย:',
  'LBL_IMPORT_VCARD' => 'นำเข้า vCard',
  'LBL_IMPORT_VCARD_SUCCESS' => 'สร้างเป้าหมายจาก vCard สำเร็จแล้ว',
  'LBL_IMPORT_VCARDTEXT' => 'สร้างเป้าหมายใหม่อัตโนมัติโดยนำเข้า vCard จากระบบไฟล์ของคุณ',
  'LBL_DUPLICATE' => 'เป้าหมายอาจซ้ำกัน',
  'MSG_SHOW_DUPLICATES' => 'ระเบียนเป้าหมายที่คุณสร้างอาจซ้ำกับระเบียนเป้าหมายที่มีอยู่แล้ว ด้านล่างนี้เป็นรายการของระเบียนเป้าหมายที่มีชื่อและ/หรือที่อยู่อีเมลคล้ายกัน<br>คลิกที่สร้างเป้าหมายเพื่อสร้างเป้าหมายใหม่นี้ต่อ หรือเลือกเป้าหมายที่มีอยู่จากรายการด้านล่าง',
  'MSG_DUPLICATE' => 'ระเบียนเป้าหมายที่คุณต้องการสร้างอาจซ้ำกับระเบียนเป้าหมายที่มีอยู่แล้ว ด้านล่างนี้เป็นรายการของระเบียนเป้าหมายที่มีชื่อและ/หรือที่อยู่อีเมลคล้ายกัน<br>คุณสามารถคลิกบันทึกเพื่อสร้างเป้าหมายใหม่นี้ต่อไป หรือคลิกยกเลิกเพื่อกลับสู่โมดูลโดยไม่สร้างเป้าหมาย',
  'LNK_IMPORT_VCARD' => 'สร้างเป้าหมายจาก vCard',
  'LNK_NEW_ACCOUNT' => 'สร้างบัญชี',
  'LNK_NEW_OPPORTUNITY' => 'สร้างโอกาสทางการขาย',
  'LNK_NEW_CASE' => 'สร้างเคส',
  'LNK_NEW_NOTE' => 'สร้างบันทึกหรือไฟล์แนบ',
  'LNK_NEW_CALL' => 'ล็อกการโทร',
  'LNK_NEW_EMAIL' => 'เก็บถาวรอีเมล',
  'LNK_NEW_MEETING' => 'วางกำหนดการประชุม',
  'LNK_NEW_TASK' => 'สร้างงาน',
  'LNK_NEW_APPOINTMENT' => 'สร้างการนัดหมาย',
  'LNK_IMPORT_PROSPECTS' => 'นำเข้าเป้าหมาย',
  'NTC_DELETE_CONFIRMATION' => 'คุณแน่ใจหรือไม่ว่าต้องการลบระเบียนนี้',
  'NTC_REMOVE_CONFIRMATION' => 'คุณแน่ใจหรือไม่ว่าต้องการย้ายที่อยู่ติดต่อนี้ออกจากเคส',
  'NTC_REMOVE_DIRECT_REPORT_CONFIRMATION' => 'คุณแน่ใจหรือไม่ว่าต้องการยกเลิกระเบียนนี้ในฐานะผู้ใต้บังคับบัญชา',
  'ERR_DELETE_RECORD' => 'ต้องระบุเลขที่ระเบียนเพื่อลบที่อยู่ติดต่อ',
  'NTC_COPY_PRIMARY_ADDRESS' => 'คัดลอกที่อยู่หลักไปยังที่อยู่รอง',
  'NTC_COPY_ALTERNATE_ADDRESS' => 'คัดลอกที่อยู่รองไปยังที่อยู่หลัก',
  'LBL_SALUTATION' => 'คำนำหน้า',
  'LBL_SAVE_PROSPECT' => 'บันทึกเป้าหมาย',
  'LBL_CREATED_OPPORTUNITY' =>'สร้างโอกาสทางการขายใหม่แล้ว',
  'NTC_OPPORTUNITY_REQUIRES_ACCOUNT' => 'การสร้างโอกาสทางการขายจะต้องมีบัญชี\\n โปรดสร้างบัญชีใหม่หรือเลือกบัญชีที่มีอยู่',
  'LNK_SELECT_ACCOUNT' => "เลือกบัญชี",
  'LNK_NEW_PROSPECT' => 'สร้างเป้าหมาย',
  'LNK_PROSPECT_LIST' => 'ดูเป้าหมาย',
  'LNK_NEW_CAMPAIGN' => 'สร้างแคมเปญ',
  'LNK_CAMPAIGN_LIST' => 'แคมเปญ',
  'LNK_NEW_PROSPECT_LIST' => 'สร้างรายการเป้าหมาย',
  'LNK_PROSPECT_LIST_LIST' => 'รายการเป้าหมาย',
  'LNK_IMPORT_PROSPECT' => 'นำเข้าเป้าหมาย',
  'LBL_SELECT_CHECKED_BUTTON_LABEL' => 'เลือกเป้าหมายที่ทำเครื่องหมาย',
  'LBL_SELECT_CHECKED_BUTTON_TITLE' => 'เลือกเป้าหมายที่ทำเครื่องหมาย',
  'LBL_INVALID_EMAIL'=>'อีเมลไม่ถูกต้อง:',
  'LBL_DEFAULT_SUBPANEL_TITLE'=>'เป้าหมาย',
  'LBL_PROSPECT_LIST' => 'รายการลูกค้าเป้าหมาย',
  'LBL_CONVERT_BUTTON_KEY' => 'V',
  'LBL_CONVERT_BUTTON_TITLE' => 'แปลงเป้าหมาย',
  'LBL_CONVERT_BUTTON_LABEL' => 'แปลงเป้าหมาย',
  'LBL_CONVERTPROSPECT'=>'แปลงเป้าหมาย',
  'LNK_NEW_CONTACT'=>'ที่อยู่ติดต่อใหม่',
  'LBL_CREATED_CONTACT'=>"สร้างที่อยู่ติดต่อใหม่แล้ว",
  'LBL_BACKTO_PROSPECTS'=>'กลับไปยังเป้าหมาย',
  'LBL_CAMPAIGNS'=>'แคมเปญ',
  'LBL_CAMPAIGN_LIST_SUBPANEL_TITLE'=>'ล็อกของแคมเปญ',
  'LBL_TRACKER_KEY'=>'คีย์ของเครื่องมือติดตาม',
  'LBL_LEAD_ID'=>'ID ผู้สนใจ',
  'LBL_LEAD' => 'ผู้สนใจ',
  'LBL_CONVERTED_LEAD'=>'ผู้สนใจที่แปลงแล้ว',
  'LBL_ACCOUNT_NAME'=>'ชื่อบัญชี',
  'LBL_EDIT_ACCOUNT_NAME'=>'ชื่อบัญชี:',
  'LBL_CREATED_USER' => 'ผู้ใช้ที่สร้าง',
  'LBL_MODIFIED_USER' => 'ผู้ใช้ที่แก้ไข',
  'LBL_CAMPAIGNS_SUBPANEL_TITLE' => 'แคมเปญ',
  'LBL_HISTORY_SUBPANEL_TITLE'=>'ประวัติ',
  //For export labels
  'LBL_PHONE_HOME' => 'โทรเข้าบ้าน',
  'LBL_PHONE_MOBILE' => 'โทรเข้ามือถือ',
  'LBL_PHONE_WORK' => 'โทรเข้าที่ทำงาน',
  'LBL_PHONE_OTHER' => 'โทรเข้าหมายเลขอื่น',
  'LBL_PHONE_FAX' => 'โทรเข้าหมายเลขโทรสาร',
  'LBL_CAMPAIGN_ID' => 'ID แคมเปญ',
  'LBL_EXPORT_ASSIGNED_USER_NAME' => 'ชื่อผู้ใช้ที่ระบุ',
  'LBL_EXPORT_ASSIGNED_USER_ID' => 'ID ผู้ใช้ที่ระบุ',
  'LBL_EXPORT_MODIFIED_USER_ID' => 'แก้ไขโดย ID',
  'LBL_EXPORT_CREATED_BY' => 'สร้างโดย ID',
  'LBL_EXPORT_EMAIL2'=>'ที่อยู่อีเมลอื่น',
  'LBL_RECORD_SAVED_SUCCESS' => 'คุณสร้าง {{moduleSingularLower}} <a href="#{{buildRoute model=this}}">{{full_name}}</a> สำเร็จแล้ว',
    //D&B Principal Identification
    'LBL_DNB_PRINCIPAL_ID' => 'ID ตัวการของ D&B',
    //Document title
    'TPL_BROWSER_SUGAR7_RECORDS_TITLE' => '{{module}} &raquo; {{appId}}',
    'TPL_BROWSER_SUGAR7_RECORD_TITLE' => '{{#if last_name}}{{#if first_name}}{{first_name}} {{/if}}{{last_name}} &raquo; {{/if}}{{module}} &raquo; {{appId}}',

    // Help Text
    // List View Help Text
    'LBL_HELP_RECORDS' => 'โมดูล {{module_name}} ประกอบด้วยบุคคลที่ไม่มีคุณสมบัติที่จะเป็นลูกค้าเป้าหมาย ซึ่งคุณมีข้อมูลบางส่วน แต่ยังไม่ใช่ {{leads_singular_module}} ที่มีคุณสมบัติตามเกณฑ์ ข้อมูล (เช่น ชื่อ ที่อยู่อีเมล) เกี่ยวกับ {{plural_module_name}} เหล่านี้มักจะได้มาจากนามบัตรที่คุณได้รับขณะไปร่วมงานแสดงสินค้า การประชุมต่างๆ เป็นต้น {{plural_module_name}} ใน Sugar เป็นระเบียนแบบสแตนด์อโลน เนื่องจากไม่มีความเกี่ยวข้องกับ {{contacts_module}}, {{leads_module}}, {{accounts_module}} หรือ {{opportunities_module}} คุณสามารถสร้าง {{plural_module_name}} ใน Sugar ได้หลายวิธี เช่น ผ่านโมดูล {{plural_module_name}} การนำเข้า {{plural_module_name}} เป็นต้น เมื่อมีการสร้างระเบียน {{module_name}} ขึ้นแล้ว คุณสามารถดูและแก้ไขข้อมูลเกี่ยวกับ {{module_name}} ได้ผ่านทางมุมมองระเบียน {{plural_module_name}}',

    // Record View Help Text
    'LBL_HELP_RECORD' => 'โมดูล {{module_name}} ประกอบด้วยบุคคลที่ไม่มีคุณสมบัติที่จะเป็นลูกค้าเป้าหมาย ซึ่งคุณมีข้อมูลบางส่วน แต่ยังไม่ใช่ {{leads_singular_module}} ที่มีคุณสมบัติตามเกณฑ์

- แก้ไขฟิลด์ของระเบียนด้วยการคลิกแต่ละฟิลด์หรือคลิกที่ปุ่มแก้ไข
- ดูหรือแก้ไขลิงก์ไปยังระเบียนอื่นในแผงข้อมูลย่อย โดยสลับแผงด้านซ้ายล่างเป็น "มุมมองข้อมูล"
- ป้อนและดูความคิดเห็นของผู้ใช้ และบันทึกประวัติการแก้ไขใน {{activitystream_singular_module}} โดยสลับแผงด้านซ้ายล่างเป็น "สตรีมกิจกรรม"
- ติดตามหรือกำหนดระเบียนนี้เป็นรายการโปรดโดยใช้ไอคอนทางขวาของชื่อระเบียน
- การดำเนินการอื่นๆ ที่ใช้ได้อยู่ในเมนูการดำเนินการแบบดรอปดาวน์ทางขวาของปุ่มแก้ไข',

    // Create View Help Text
    'LBL_HELP_CREATE' => 'โมดูล {{module_name}} ประกอบด้วยบุคคลที่ไม่มีคุณสมบัติที่จะเป็นลูกค้าเป้าหมาย ซึ่งคุณมีข้อมูลบางส่วน แต่ยังไม่ใช่ {{leads_singular_module}} ที่มีคุณสมบัติตามเกณฑ์

ในการสร้าง {{module_name}} ให้ทำดังนี้:
1. ระบุค่าสำหรับฟิลด์ตามที่ต้องการ
 - คุณต้องป้อนข้อมูลในฟิลด์ที่มีคำว่า "ต้องระบุ" กำกับไว้ ก่อนที่จะบันทึก
 - คลิก "แสดงเพิ่มเติม" เพื่อแสดงฟิลด์อื่นๆ หากจำเป็น
2. คลิก "บันทึก" เพื่อสิ้นสุดการสร้างระเบียนใหม่ และกลับสู่เพจก่อนหน้า',

    'LBL_FILTER_PROSPECTS_REPORTS' => 'รายงานของเป้าหมาย',
);
