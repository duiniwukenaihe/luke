    ...
    class OutfittersLicense
    {
        ...
        {	
            ...
            if(!empty($user_id) && $outfitters_config['validate_users'] == true)
            {
                ...
                if(empty($row)) {
                   return 'You do not have access to this Module. Go to Admin->RT SalesMap Users Configuration';
                }
            }
                ...
                if(($last_validation['last_ran'] + $elapsed) >= time()) {
                    ...
                    if($last_validation['last_result']['success'] === false) {
                        ...
                        if($validation_result === false) {
                            ...
                            return 'Insuffient number of user licenses. Please add additional user licenses and validate again.';
                        } else {
                            return true;
                        }
                    }
        ...
        {
            return array('data'=>$validated['success']); // uncomment this line for licensed version
            ...
            if($validated['success'] == 1) {             		 
			   ...
			   require('modules/'.$currentModule.'/license/config.php');
				...
				//   header('HTTP/1.1 400 Bad Request');	
				// this case will occurred when key is invalid
            }
			
           // echo $json->encode($validated['result']);
            return array('data'=>$validated['success']);
        }
		...
		$json = getJSONobj();
            ...
            {
                require_once('modules/Administration/Administration.php');
		require('modules/rt_maps/license/config.php');
		...
		$store = array(
          ...
          //  echo $json->encode($response['result']);
          //  exit;
		return array('data'=>$response['result']);
        }
    ...
    class SugarOutfitters_API
    {
        ...
        public static function call($module,$path,$custom_data=array(),$method='post')
        {
            ...
            if(empty($data['key']))
            {
                return array(
                    'success' => false,
                    'result' => 'Key could not be found locally. Please go to the Admin->RT SalesMap License Verification and enter your key.'
                );
            }
